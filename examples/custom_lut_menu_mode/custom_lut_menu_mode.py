...
CDLS = ["cc", "ccc", "cdl"]
LUTS = ["3dl", "a3d", "csp", "cube", "mga", "rv3dlut",
        "rvchlut", "rvlut", "txt", "vf"]
...
class CustomLUTMenuMode(rvtypes.MinorMode):
    ...
    def setDefaultDisplayLUT(self, event): 
        deflut = commands.getStringProperty(
            self.lutNodes["DLUT"] + ".lut.file")[0]
        if deflut != '':
    ...
    def toggleDefaultDisplayLUT(self, event):
        ...
        if self._autoLoadDLUT:
            ...
            extra_commands.displayFeedback(
                "Default Display LUT: %s" % (deflut), 5.0)
    ...
    def applyOnAllDisplays(self, event):
        ...
        if self._onAllDisplays:
            lut = commands.getStringProperty(
                self.lutNodes["DLUT"] + ".lut.file")[0]
    ...
    def restoreDefaultProps(self, lutNode, display=False):
        ...
        if (linNode in self._setups.keys()):
            for prop in intProps:
                commands.setIntProperty(linNode + ".color." + prop,
                    [int(self._setups[linNode][prop])], True)
            for prop in fltProps:
                commands.setFloatProperty(linNode + ".color." + prop,
                    [float(self._setups[linNode][prop])], True)
    ...
    def setLUT(self, path, lutNode, display=False):
        ...
        try:
            ...
            print("ERROR: Failed to set LUT '%s' for %s: %s" %
        ...
        for lut in self.lutNodes.keys():
            if (self.lutNodes[lut].startswith("#") and self._autoLoadCXfrm[lut]):
                if (self._autoFormat[lut] != ""):
                    print("INFO: Searching for accompanying '%s'" %
    ...
    def displayColorSetup(self, event):
        ...
        for dnode in dnodes:
            ...
            if self._autoLoadDLUT:
                lut = commands.readSettings(
                    "CUSTOM_LUTS", "defaultDisplayLut", "NONE")
                if lut != "NONE":
    ...
    def toggleCXfrm(self, event):
        for source in commands.nodesOfType("RVSourceGroup"):
            for lut in self.lutNodes.keys():
                ...
                try:
                    ...
                    defCDL = (slope == [1,1,1] and offset == [0,0,0] and
                        power == [1,1,1] and sat == 1)
                except:
                    defCDL = True
                ...
                if not defLUT or not defCDL:
                    try:
                        commands.setIntProperty(xfrmNode + ".CDL.active",
                            [int(not self._activeCXfrm)], True)
                    except:
                        pass
        ...
        extra_commands.displayFeedback("Custom Color is now %s" %
            ("active" if self._activeCXfrm else "inactive"), 5.0)
    ...
    def loadColorXfrm(self, lut, event):
        ...
        if self._autoLevel[lut] < 0:
            try:
                ...
                print("ERROR: Unable to determine extension from '%s'" %
                    xfrmFile)
        else:
            dirUp = 0
        ...
        if xfrmFile == None:
            extra_commands.displayFeedback("Unable to find accompanying color" +
                " transform with extension '%s'" % self._autoFormat[lut], 5.0)
            print("WARNING: No '%s' file found in '%s' for '%s'" %
        ...
        if xfrmExt in CDLS:
            try:
                cdl = xml.dom.minidom.parse(xfrmFile)

                def getvals(dom, param):
                    v = dom.getElementsByTagName(param)[0]
                    return v.firstChild.nodeValue
                s = getvals(cdl, "Slope").split()
                o = getvals(cdl, "Offset").split()
                p = getvals(cdl, "Power").split()
                sat = getvals(cdl, "Saturation").split()

                if (len(s) == len(o) == len(p) == 3 and len(sat) == 1):
                    msg = "INFO: Applying CDL\n{"
                    msg += "\n\tfloat[3] slope = [ [ %s %s %s ] ]" % tuple(s) 
                    msg += "\n\tfloat[3] offset = [ [ %s %s %s ] ]" % tuple(o)
                    msg += "\n\tfloat[3] power = [ [ %s %s %s ] ]" % tuple(p)
                    msg += "\n\tfloat saturation = %s\n}" % sat[0]
                    print(msg)

                    commands.setFloatProperty(xfrmNode + ".CDL.slope",
                        [float(s[0]), float(s[1]), float(s[2])], True)
                    commands.setFloatProperty(xfrmNode + ".CDL.offset",
                        [float(o[0]), float(o[1]), float(o[2])], True)
                    commands.setFloatProperty(xfrmNode + ".CDL.power",
                        [float(p[0]), float(p[1]), float(p[2])], True)
                    commands.setFloatProperty(xfrmNode + ".CDL.saturation",
                        [float(sat[0])], True)
                    commands.setIntProperty(xfrmNode + ".CDL.active",
                        [int(self._activeCXfrm)], True)
                    lutNode = extra_commands.associatedNode("RVLookLUT",srcNode)
                    self.saveDefaultProps(lutNode)
                else:
                    extra_commands.displayFeedback(
                        "CDL File Incomplete: Settings Unchanged", 5.0)

            except Exception,inst:
                extra_commands.displayFeedback(
                    "CDL File Failed to Read: Check File %s" % xfrmFile, 5.0)
                print("ERROR: Failed to set CDL '%s' for %s: %s" %
    ...
    def setAutoFormat(self, lut, ext):
        def saveSetting(event):
            ...
            commands.writeSettings("CUSTOM_LUTS", lut + "loadColorTransform",
                self._autoLoadCXfrm[lut])
            if self._autoLoadCXfrm[lut]:
                extra_commands.displayFeedback(
                    "Autoload shot CDL/Look LUT: %s" % (ext), 5.0)
                commands.writeSettings("CUSTOM_LUTS", lut+ "autoColorTransform",
                    self._autoFormat[lut])
            else:
                extra_commands.displayFeedback(
                    "Disabled Autoload CDL/Look LUT", 5.0)
        return saveSetting
            ...
            if level < 0:
                try:
                    self._autoLUT[lut] = commands.openFileDialog(
                        True, False, False, None, self._dirs[lut])[0]
                    commands.writeSettings("CUSTOM_LUTS",
                        lut + "AutoLut", self._autoLUT[lut])
                    self.setAutoFormat(lut, "")(None)
                except:
                    return
            self._autoLevel[lut] = level
            commands.writeSettings("CUSTOM_LUTS", lut + "autoFormatDirLevel",
                self._autoLevel[lut])
    ...
    def listSupportedFormats(self, lut, allowCDL=False):
        ...
        for directory,level in [("Same Directory", 0), ("Parent Directory", 1), ("Fixed Location...", -1)]:
            fmtOpts.append(("  %s" % directory,
                self.setAutoFormatLevel(lut, level), "", self.thisLevel(lut, level)))
        fmtOpts += [("_", None)]
        ...
        if allowCDL:
            ...
            for ext in CDLS:
                title = "  " + ext.upper()
                fmtOpts.append((title, self.setAutoFormat(lut, ext), "",
        ...
        fmtOpts += [("_", None),
            ("None/Off", self.setAutoFormat(lut, None), "",
    ...
    def buildLUTMenu(self, typeStr):
        ...
        if (self._dirs[typeStr] == ""):
            luts = [("RV_CUSTOM_%s_DIR Undefined" % typeStr, None,
                None, lambda: commands.DisabledMenuState)]
        else:
            ...
            if os.path.exists(self._dirs[typeStr]):
                ...
                for lut in dircontents:
                    ...
                    if ext in LUTS:
                        ...
                        luts.append((lut, self.loadLUT(path, lutNode), "",
                            self.isLoadedLUT(path, lutNode)))
            luts += [("_", None),
                ("Clear LUT Selection", self.loadLUT(None, lutNode), "", None)]

        luts += [("_", None), ("Change %s Directory..." % typeStr,
               self.changeLUTDir(typeStr), "", None)]
        return luts
        ...
        def changeSpecificLUTDir(event):
            try:
                self._dirs[typeStr] = commands.openFileDialog(
                    True, False, True, None, None)[0]
                commands.defineModeMenu(
                    "custom-luts-mode", self.createMenu(), True)
                commands.writeSettings(
    ...
    def createMenu(self):
      return [ ("Custom LUT", 
          [("Display", None, None, lambda: commands.DisabledMenuState),
           ...
           ("  Auto Load Default Display LUT On Launch", self.toggleDefaultDisplayLUT,
               "", self.isAutoLoadingDisplayLUT),
           ("  Force View->No Correction On Launch",
               self.toggleNoLinearToDisplay,
               "", self.isToDisplay),
           ("  Apply To All Displays", self.applyOnAllDisplays,
               "", self.onAllDisplays),
           ...
           ("  Custom Source Assignments Enabled", self.toggleCXfrm, "/",
               self.isActiveCXfrm),
           ("  Force Color->No Conversion On Load", self.toggleNoLinearization,
               "", self.isToLinearNoConversion),
           ("_",None),
           ("Prefer Environment LUT_DIR Over Last Used", self.saveDirPref,
               "", self.preferEnvVar)
         ] ) ]
    ...
    def __init__(self):
        ...
        for lut in self.lutNodes.keys():
            self._autoLoadCXfrm[lut] = commands.readSettings("CUSTOM_LUTS",
                lut+ "loadColorTransform", False)
            self._autoFormat[lut] = commands.readSettings("CUSTOM_LUTS",
                lut + "autoColorTransform", "")
            self._autoLevel[lut] = commands.readSettings("CUSTOM_LUTS",
                lut + "autoFormatDirLevel", 0)
            self._autoLUT[lut] = commands.readSettings("CUSTOM_LUTS",
                lut + "AutoLut", "NONE")
        ...
        for typeStr in self._dirs.keys():
            ...
            customSettingDir = commands.readSettings("CUSTOM_LUTS",
                "custom%sDir" % typeStr, customEnvDir)
            self._dirs[typeStr] = customSettingDir
            if (self._preferEnv and customEnvDir != ""):
                self._dirs[typeStr] = customEnvDir

        self.init("custom-luts-mode",
                  [("source-group-complete", self.sourceColorSetup,
                    "Source Color Setup"),
                   ("after-progressive-loading", self.displayColorSetup,
                    "Display Color Setup"),
                   ("session-initialized", self.displayColorSetup,
                    "Display Color Setup")],
                  None, self.createMenu(),"zzz")

        commands.bind(
            "custom-luts-mode", "global", "key-down--/", self.toggleCXfrm, "")
